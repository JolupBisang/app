...
class AudioRepositoryImpl @Inject constructor(
    ...
    override suspend fun embeddingAudio(audioFile: java.io.File): ApiResult<Unit> = withContext(Dispatchers.IO) {
        try {
            val requestFile = audioFile.asRequestBody("audio/mp4".toMediaTypeOrNull())
            val multipart = MultipartBody.Part.createFormData("audioFile", audioFile.name, requestFile)
    ...
    override suspend fun getAudioList(meetingId: Long): ApiResult<List<AudioInfo>> = withContext(Dispatchers.IO) {
        try {
